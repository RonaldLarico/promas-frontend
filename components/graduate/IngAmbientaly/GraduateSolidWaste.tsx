...
import { modulesSolidWaste } from "@/components/utils/courses/course";
import { BiChevronDown, BiChevronUp } from "react-icons/bi";
import { FaRegFilePdf, FaWhatsapp } from "react-icons/fa6";
import Modal  from "@/components/share/Modal";
import Image from "next/image";

const GraduateSolidWaste = () => {
...
const [imageUrl, setImageUrl] = useState('');
  ...
  const handleClick = () => {
    const newWindow = window.open('/BROCHURE/GESTIÓN_MANEJO_INTEGRAL_RESIDUOS_SÓLIDOS.pdf');
    setOpenWindow(newWindow);

    // Descargar el archivo PDF
    const link = document.createElement('a');
    link.href = '/BROCHURE/GESTIÓN_MANEJO_INTEGRAL_RESIDUOS_SÓLIDOS.pdf';
    link.download = 'GESTIÓN_MANEJO_INTEGRAL_RESIDUOS_SÓLIDOS.pdf';
    link.click();
  };

  return (
    <div id="solidWaste" className="">
              ...
              <div className="grid lg:grid-cols-3 md:grid-cols-1 grid-cols-1 gap-2">
                <div className="w-full">
                  <Link
                    ...
                    href='https://wa.me/+51957033871?text=Hola,%20deseo%20más%20información%20sobre%20el%20diplomado%20de%20Gestión%20y%20Manejo%20Integral%20de%20Residuos%20Sólidos'
                    target="_blank">
            ...
            {isAccordionOpen1 && (
              <motion.div
                ...
                <div className="text-sm font-sans font-semibold">
                  <ul className="">
                  {modulesSolidWaste.map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0 }}
...
export default GraduateSolidWaste;
